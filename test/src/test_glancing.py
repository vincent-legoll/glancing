...
class TestGlancingImage(unittest.TestCase):
    ...
    _TTYLINUX_FN = 'ttylinux-16.1-x86_64.img'
    _TTYLINUX_FILE = get_local_path('..', 'images', _TTYLINUX_FN)
    ...
    def glancing_test_image_devnull_bad_sum(self):
        self.assertFalse(glancing.main(['-d', 'image', os.devnull, '-s',
            '### BAD CHECKSUM ###']))
    ...
    def glancing_test_image_devnull(self):
        self.assertTrue(glancing.main(['-d', 'image', os.devnull, '-s',
            self._DEVNULL_MD5]))

    def glancing_test_image_devnull_verbose(self):
        self.assertTrue(glancing.main(['-dv', 'image', os.devnull, '-s',
            self._DEVNULL_MD5]))
    ...
    def glancing_test_image_one(self):
        ...
        for chks in checksums_true:
            self.assertTrue(glancing.main(['-d', 'image',
        ...
        for chks in checksums_false:
            self.assertFalse(glancing.main(['-d', 'image',
                self._TTYLINUX_FILE, '-s', chks]), chks)
    ...
    def glancing_test_image_import_noname(self):
        ...
        with cleanup(['glance', 'image-delete', name]):
            self.assertTrue(glancing.main(['-f', 'image',
    ...
    def glancing_test_image_import_name_force(self):
        with cleanup(['glance', 'image-delete', test_name()]):
            self.assertTrue(glancing.main(['-f', '-n', test_name(),
                'image', self._TTYLINUX_FILE, '-s', '0' * 32]))
    ...
    def glancing_test_metadata_heavies(self):
        ...
        for market_id, status_json, status_market in market_ids:
            mdfile = get_local_path('..', 'stratuslab', market_id +
                '.json')
            self.assertEqual(status_json, glancing.main(['-d', 'json',
                mdfile]), mdfile)
            self.assertEqual(status_market, glancing.main(['-d', 'market',
                market_id]), market_id)
...
class BaseGlancingUrl(unittest.TestCase):
    ...
    _CIRROS_FN = 'cirros-0.3.4-i386-disk.img'
    _CIRROS_URL_DIR = 'http://download.cirros-cloud.net/0.3.4/'
    _CIRROS_URL = _CIRROS_URL_DIR + _CIRROS_FN
...
class TestGlancingUrlImport(BaseGlancingUrl):
    ...
    def glancing_test_url_import_bad_md5(self):
        with cleanup(['glance', 'image-delete', test_name()]):
            self.assertFalse(glancing.main(['-n', test_name(), 'url',
                self._CIRROS_URL, '-s', '0' * 32]))

    def glancing_test_url_import_bad_md5_but_force(self):
        with cleanup(['glance', 'image-delete', test_name()]):
            self.assertTrue(glancing.main(['-f', '-n', test_name(),
                'url', self._CIRROS_URL, '-s', '0' * 32]))

    def glancing_test_url_import_no_md5(self):
        with cleanup(['glance', 'image-delete', test_name()]):
            self.assertTrue(glancing.main(['-n', test_name(), 'url',
                self._CIRROS_URL]))
